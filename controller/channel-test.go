	...
	go func() {
		for _, channel := range channels {
			...
			if isChannelEnabled && milliseconds > disableThreshold {
				err = fmt.Errorf("response time of %.2fs exceeds the threshold of %.2fs", float64(milliseconds)/1000.0, float64(disableThreshold)/1000.0)
		...
		testAllChannelsLock.Unlock()
		if notify {
			err := message.Notify(message.ByAll, "Channel testing complete!", "", "Channel testing is complete! If you haven't received a disable notification, it means all channels are working normally.")
			if err != nil {
