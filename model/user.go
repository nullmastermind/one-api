...
func (user *User) Insert(ctx context.Context, inviterId int) error {
	...
	if config.QuotaForNewUser > 0 {
		RecordLog(ctx, user.Id, LogTypeSystem, fmt.Sprintf("New users get %s upon registration!", common.LogQuota(config.QuotaForNewUser)))
	}
	if inviterId != 0 {
		if config.QuotaForInvitee > 0 {
			_ = IncreaseUserQuota(user.Id, config.QuotaForInvitee)
			RecordLog(ctx, user.Id, LogTypeSystem, fmt.Sprintf("Use invite code to get %s!", common.LogQuota(config.QuotaForInvitee)))
		}
		if config.QuotaForInviter > 0 {
			_ = IncreaseUserQuota(inviterId, config.QuotaForInviter)
			RecordLog(ctx, inviterId, LogTypeSystem, fmt.Sprintf("Invite users to gift %s", common.LogQuota(config.QuotaForInviter)))
		}
	}
...
func ResetUserPasswordByEmail(email string, password string) error {
	if email == "" || password == "" {
		return errors.New("oops! Email or password was empty")
	}
